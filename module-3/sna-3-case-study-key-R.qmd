...
subtitle: "SNA Module 3: Case Study Key"
...
-   **Transmitters:** who tweeted a lot, regardless of the extent of their followership

-   **Transceivers:** those who gained their influence by being frequently retweeted and mentioned

-   **Transcenders:** those who were both transmitters and transreceivers
...
library(tidygraph)
library(ggraph)
library(skimr) #make summaries of dataset
library(igraph)

pacman::p_load(ggplot2, tidyverse, readr, igraph, tidygraph, ggraph, janitor)
```
...
b.  **Format Network Data**. We learn to shape our data files into two common formats for storing network data: edgelists and nodelists.
...
```{r unnest-tokens}
ties_2 <- ties_1 %>%
  unnest_tokens(input = target, #unnest the token
                output = receiver,
                to_lower = FALSE) %>%
...
#YOUR CODE HERE
ties <- ties_2 %>%
  drop_na(receiver) #remove the rows with missing values
```
...
actors <- actors_1 %>%
  select(value) %>% 
  rename(actors = value) %>% 
  distinct() #find unique names and remove duplicates
```
...
-   92 unique nodes/actors in our CCSS network
...
#need to check the number of edges and nodes
```
...
#weak is treating as network chunks are undirected
#strong is treating as network chunks are directed
#no right answer whether its weak or strong - but our network is directed
```
...
```{r clique_number}
clique_num(ccss_network) #warning message - larger than 3
```
...
```{r cliques, eval=FALSE}
cliques(ccss_network, min = 4, max = NULL) #we hace no cliques that contain a minmum of 4 nodes
```
...
ccss_network |> 
  as_tibble() |>
  ggplot() +
  geom_histogram(aes(x = in_degree)) #usual degree of distribution
```
...
ccss_network |> 
  as_tibble() |>
  skim() #similiar to summarize() provides summar stats
```
...
ccss_network |> 
  as_tibble() |>
  filter(size < 51) |> #those with less than 51 alters
  group_by(weak_component) |>
  select(size) |>
  skim()
```
