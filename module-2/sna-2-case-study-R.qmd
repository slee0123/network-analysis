...
1.  **Prepare**: Prior to analysis, we'll take a look at the context from which our data came and load various R packages for data science and network analysis.

2.  **Wrangle**: In section 2 we make use of the {readr} and {tidygraph} packages to import and prepare relational data stored as as an adjacency matrix.

3.  **Explore**: In our prior case study, we indirectly used the {igraph} package on which {tidygraph} depends. In section 3, we use {igraph} directly to calculate a range of network-level measures to describe and compare collaboration networks over time.
...
Both {tidygraph} and {ggraph} depend heavily [igraph network analysis package](https://igraph.org/). The main goals of the igraph package and the collection of network analysis tools it contains are to provide a set of data types and functions for:
...
In general, data wrangling involves some combination of cleaning, reshaping, transforming, and merging data[@wickham2016r]. As highlighted in Estrellado et al. @estrellado2020e, wrangling network data can be even more challenging than other data sources since network data often includes variables about both individuals and their relationships. For data wrangling in this lab, we're keeping it simple since working with relational data, especially in the form of matrices, is a bit of a departure from our working with rectangular data frames.
...
Relevant to our case study of the DLT 1 & 2 MOOC-Eds, Carolyn notes in SNA and Education @carolan2014 that:
...
Recall from our previous SNA case study examining student friendships in middle school that we introduced the dead simple [`plot()`](https://www.rdocumentation.org/packages/graphics/versions/3.6.2/topics/plot) function built into R, as well as the \`autograph()\` function from the {ggraph} package to quickly generate a sociogram showing the nodes and edges in our network.

Let's supply our `dlt1-network` object as the first argument to each of these functions and see what they produce:
...
-   [`geom_edge_*()`](https://ggraph.data-imaginist.com/articles/Edges.html) functions and their associated arguments add and modify the edges of the network plot.
...
Run the following code chunk to use the [`gorder()`](https://igraph.org/r/html/latest/gorder.html) and [`gsize()`](https://igraph.org/r/html/latest/gsize.html) functions obtain the number of vertices and edges respectively:
...
The [`centr_degree()`](https://igraph.org/r/html/latest/centr_degree.html) function in {igraph} provides a centralization score based on the metric selected, as well as the degree for each actors in the network. Let's first take a look at the centralization measure based on number of actors individuals have ties with regardless of directionality:
...
The igraph packages has two different function for calculating the density of a network, [`edge_density()`](https://igraph.org/r/html/latest/edge_density.html) and [`graph.density()`](https://igraph.org/r/html/latest/edge_density.html), but both yield the exact same result. The `edge_density()` function is the more recent function and should be used, but let's try them both out:
...
And if we wanted to highlight this lack of reciprocity in a network visual, we could use the [`edge_is_mutual()`](https://tidygraph.data-imaginist.com/reference/edge_types.html) function to create a new edge variable called `reciprocated` that indicates whether the tie is mutual or asymmetric.
...
1.  How do network-level measures for DLT 1 and DLT 3 collaboration compare? What are some similarities and differences?
...
As highlighted in [Chapter 3 of Data Science in Education Using R](https://datascienceineducation.com/c03.html), the **Model** step of the data science process entails "using statistical models, from simple to complex, to understand trends and patterns in the data."
